# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/)
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [0.2.11-alpha] - unreleased

This is an alpha version! The changes listed here are not final.

## [0.2.10] - 2026-09-23
### Changed
- Update dependencies.

## [0.2.9] - 2026-09-18
### Changed
- Update package dependencies. [#52187]

## [0.2.8] - 2026-09-15
### Changed
- Update package dependencies. [#52297]

## [0.2.7] - 2026-09-14
### Changed
- Update dependencies.

## [0.2.6] - 2026-09-08
### Changed
- Update package dependencies. [#51701]

## [0.2.5] - 2026-09-01
### Changed
- Declare react-dom, which @wordpress/element now requires as a peer dependency. [#51303]
- Update package dependencies. [#51802]

## [0.2.4] - 2026-08-06
### Changed
- Update dependencies.

## [0.2.3] - 2026-07-31
### Changed
- Update dependencies.

## [0.2.2] - 2026-07-27
### Changed
- Update dependencies.

## [0.2.1] - 2026-07-22
### Changed
- Update dependencies.

## [0.2.0] - 2026-07-20
### Added
- Export mapAiFeatureResponseToAiFeatureProps from the package barrel so consumers can hydrate the wordpress-com/plans store from a server-side preload of the ai-assistant-feature payload. [#50494]

### Changed
- Update package dependencies. [#50510] [#50529]

## [0.1.5] - 2026-07-13
### Changed
- Update dependencies.

## [0.1.4] - 2026-07-09
### Changed
- Update dependencies.

## [0.1.3] - 2026-07-06
### Changed
- Use store objects instead of string IDs. [#49810]
- Update package dependencies. [#50097] [#50183]

## [0.1.2] - 2026-06-26
### Changed
- Internal updates.

## [0.1.1] - 2026-06-23
### Changed
- Update dependencies.

## 0.1.0 - 2026-06-22
### Added
- Add dedicated package for the externalized `jetpack-modules` and `wordpress-com/plans` data stores. [#49494]

### Changed
- Update package dependencies. [#49757]

[0.2.11-alpha]: https://github.com/Automattic/jetpack-shared-stores/compare/v0.2.10...v0.2.11-alpha
[0.2.10]: https://github.com/Automattic/jetpack-shared-stores/compare/v0.2.9...v0.2.10
[0.2.9]: https://github.com/Automattic/jetpack-shared-stores/compare/v0.2.8...v0.2.9
[0.2.8]: https://github.com/Automattic/jetpack-shared-stores/compare/v0.2.7...v0.2.8
[0.2.7]: https://github.com/Automattic/jetpack-shared-stores/compare/v0.2.6...v0.2.7
[0.2.6]: https://github.com/Automattic/jetpack-shared-stores/compare/v0.2.5...v0.2.6
[0.2.5]: https://github.com/Automattic/jetpack-shared-stores/compare/v0.2.4...v0.2.5
[0.2.4]: https://github.com/Automattic/jetpack-shared-stores/compare/v0.2.3...v0.2.4
[0.2.3]: https://github.com/Automattic/jetpack-shared-stores/compare/v0.2.2...v0.2.3
[0.2.2]: https://github.com/Automattic/jetpack-shared-stores/compare/v0.2.1...v0.2.2
[0.2.1]: https://github.com/Automattic/jetpack-shared-stores/compare/v0.2.0...v0.2.1
[0.2.0]: https://github.com/Automattic/jetpack-shared-stores/compare/v0.1.5...v0.2.0
[0.1.5]: https://github.com/Automattic/jetpack-shared-stores/compare/v0.1.4...v0.1.5
[0.1.4]: https://github.com/Automattic/jetpack-shared-stores/compare/v0.1.3...v0.1.4
[0.1.3]: https://github.com/Automattic/jetpack-shared-stores/compare/v0.1.2...v0.1.3
[0.1.2]: https://github.com/Automattic/jetpack-shared-stores/compare/v0.1.1...v0.1.2
[0.1.1]: https://github.com/Automattic/jetpack-shared-stores/compare/v0.1.0...v0.1.1
